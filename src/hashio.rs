...
pub trait HashIOImpl<T> where T: Hashtype {
    ...
    // Loader hints
    /// Define if version or type check should be disabled
    ...
    /// Check if the given type hash is valid
    fn type_hash_valid(type_hash: &Hash) -> bool {
...
macro_rules! tbd_model {
    ...
    ($model_name:ident
        ...
        impl Typeable for $model_name {
            ...
            fn type_name() -> String {
                String::from(stringify!($model_name))
            }
        }
...
impl<T, U> Typeable for BTreeMap<T, U>
    ...
    fn type_name() -> String {
        let mut res = String::from("BTreeMap<");
        res.push_str(&T::type_name());
        res.push_str(",");
        res.push_str(&U::type_name());
        res.push_str(">");
        return res
    }
}
